...
router.route('/addlecturer').post((req, res) => {
  ...
  bcrypt.hash(icNumber, 10, (err, hash) => {
    if (err) {
      ...
      const sql = "INSERT INTO lecturer(staffNo, lecturerName, icNumber, lecturerTelephoneNo,lecturerEmail,lecturerPassword, lecturerImage, lecturerImageFirebase,faculty, department, createdDate, status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
